...
MCICTL <- read.csv("C:/Users/sulei/Downloads/Tutorials/R/SL ML challenge 1/MCICTLtrain.csv")
str(MCICTL)
MCICTL <- MCICTL[-1]

# Convert label to 0 and 1 and make it a factor
MCICTL$Label <- ifelse(MCICTL$Label == "MCI", 1, 0)
MCICTL$Label <- factor(MCICTL$Label)

# Apply cross-validation
set.seed(3033)
trainsplit <- createDataPartition(y = MCICTL$Label, p = 0.7, list = FALSE)
training <- MCICTL[trainsplit, ]
testing <- MCICTL[-trainsplit, ]
...
test_ids <- MCICTLtest$ID
MCICTLtest <- MCICTLtest[-1]
testing_norm <- predict(preproc_range, MCICTLtest)
...
TP1 <- sum(prediction1 == 1 & MCICTLtest_label== 1)
TN1 <- sum(prediction1 == 0 & MCICTLtest_label == 0)
FP1 <- sum(prediction1 == 1 & MCICTLtest_label == 0)
FN1 <- sum(prediction1 == 0 & MCICTLtest_label== 1)
...
auc1 <- roc(MCICTLtest_label, as.numeric(prediction1))$auc
MCC1 <- (TP1 * TN1 - FP1 * FN1) / sqrt((TP1 + FP1) * (TP1 + FN1) * (TN1 + FP1) * (TN1 + FN1))
balanced_accuracy1 <- (sensitivity1 + specificity1) / 2

# Convert labels and predictions to factors
label <- factor(as.character(MCICTLtest_label), levels = c("0", "1"))
...
#write.csv(results, "C:/Users/sulei/Downloads/Tutorials/R/SL ML challenge 1/0075720_Suleiman_MCICTLres.csv", row.names = FALSE)
